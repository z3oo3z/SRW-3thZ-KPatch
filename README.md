# SRW-3thZ-KPatch
<img width="1234" height="620" alt="image" src="https://github.com/user-attachments/assets/49e7463e-3e3e-4dd8-a2b4-8d5112b7b860" />
-타이틀 이미지 : 베라요정님 작품-


PS3 제3차 슈퍼로봇대전Z 연옥편 한국어 패치

본 패치는 비영리 목적으로 제작되었습니다
본 패치의 상업적 이용은 금지합니다

본 패치를  재배포 하지 마십시오
본 패치의 복호키,비번을 공유하지 마십시오
본 패치를 공유하거나 롬에 입혀 공유하지 마십시오
저작권으로 인한 법적인 책임은 패치를 사용한 사용자에게 있음을 알려드립니다
 

제3차 슈퍼로봇대전Z 연옥편 
버전 : v1.0
 

[한글패치]
다운로드

[압축비번]
2015년 4월 2일+0104.7z

패치방법은 압축파일내 문서 참조하세요

[필요한 유틸]
xdelta UI
https://www.romhacking.net/utilities/598/  

xdelta 3.1.0
https://github.com/jmacd/xdelta-gpl/releases/download/v3.1.0/xdelta3-3.1.0-x86_64.exe.zip  

압축해제후 xdelta.exe로 바꾸고 덮어쓰세요

해독툴은 별도로 필요 없고 
rpcs3에뮬 메뉴/utilites/decrypt ps3 binaries에서 해독할 수 있습니다
(rap키 필요)

MD5값 확인은 각각의 파일을 md5cheker.bat 에 드래그&드롭해서 올려 두면 
MD5값이 표시됩니다


폰트는 
밝은 색 폰트로 넘어가면 글씨가 잘 안 보이는 특징이 있습니다 
대사는 잘보이니까 별 문제는 아닙니다

패치 과정은 
2개 파일의 해독과정이 필요하고 그외에는 알파 수준의 패치입니다

rpcs3 에뮬은 rpcs3-v0.0.16버전
----------------------------------------
패치가 안 되시는 분은
xdelta UI로 수동 패치하세요


EBOOT, SDAT해독 RPCS3 메뉴 유틸에서 해독하세요

